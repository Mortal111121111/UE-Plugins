...
void UK2Node_OpenUI::ExpandNode( FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph )
{
    ...
    UEdGraphPin* SelfWorldContextObjectPin = GetWorldContextObjectPin();
    if(SelfWorldContextObjectPin)
    {
        UEdGraphPin* WorldContextObjectFuncPin = OpenUIFunction->FindPinChecked( OpenUIHelper::WorldPinName );
        CompilerContext.MovePinLinksToIntermediate( *SelfWorldContextObjectPin, *WorldContextObjectFuncPin );
    }
...
{
    UEdGraphPin* Pin = FindPin( OpenUIHelper::WorldPinName );
...
void UK2Node_OpenUI::RefreshOutputPinType() const
{
    ...
    if(UINamePin != nullptr && UINamePin->DefaultValue != "None")
    {
        ...
        UEdGraphPin* ResultPin = GetResultPin();
        if(Settings->UIDataTablePath.ResolveObject() != nullptr)
        {
        ...
        else
        {
            ...
            {
                ResultPin->PinType.PinSubCategoryObject = ResultPin->LinkedTo[0]->PinType.PinSubCategoryObject.Get();
            }
        }
    }
}
